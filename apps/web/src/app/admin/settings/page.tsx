                ...
                <nav className="lg:w-48 flex flex-row lg:flex-col gap-1 overflow-x-auto shrink-0">
                    {TABS.map(({ id, label, icon: Icon }) => (
                        <button
                            ...
                            className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${activeTab === id ? 'bg-indigo-600/10 text-indigo-400 border border-indigo-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent'}`}
                        >
                        ...
                        <div className="bg-[#0B1120] border border-indigo-900/30 rounded-2xl overflow-hidden">
                            ...
                            <div className="p-6 space-y-5">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    {[
                                        { label: 'Họ và tên', placeholder: 'Nguyễn Văn A', defaultValue: 'Thầy Minh' },
                                        { label: 'Tiêu đề', placeholder: 'Giảng viên cao cấp', defaultValue: 'Giảng viên cao cấp' },
                            ...
                            {TOGGLE_SETTINGS.map(({ key, label, desc }) => (
                                <div key={key} className="flex items-center justify-between p-4 bg-[#010816] rounded-xl border border-indigo-900/30">
                                    ...
                                    <button
                                        ...
                                        className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ml-4 ${toggles[key] ? 'bg-indigo-600' : 'bg-slate-700'}`}
                                    >
