        ...
        <div className="flex flex-col h-screen bg-[#010816] text-slate-100 overflow-hidden font-sans">
            ...
            <nav className="bg-[#0B1120] border-b border-slate-800 h-16 flex items-center justify-between px-4 sm:px-6 z-20 shrink-0">
                <div className="flex items-center gap-4">
                    <Link href="/admin/dashboard" className="flex items-center gap-2 text-slate-400 hover:text-white transition-colors p-2 rounded-full hover:bg-white/5">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={handleSave} disabled={saving} className="px-4 py-2 text-sm font-medium text-slate-300 hover:text-white hover:bg-white/5 border border-transparent hover:border-slate-700 rounded-lg transition-all">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <aside className="w-72 bg-[#050C1F] border-r border-slate-800 flex flex-col overflow-y-auto shrink-0">
                    <div className="p-4 border-b border-slate-800 flex justify-between items-center sticky top-0 bg-[#050C1F]/95 backdrop-blur z-10">
                        ...
                        <button onClick={addModule} className="text-indigo-400 hover:text-white hover:bg-indigo-500/20 p-1.5 rounded transition-colors" title="Thêm chương">
                            ...
                            <div key={mod.id} className="group">
                                ...
                                {mod.expanded && (
                                    <div className="mt-1 ml-3 pl-3 border-l-2 border-slate-800 space-y-1">
                                        ...
                                        <button onClick={() => addLesson(mod.id)} className="mt-1 text-xs text-slate-500 hover:text-indigo-400 flex items-center gap-1 py-1 px-2 rounded hover:bg-white/5 transition-colors w-fit">
                    ...
                    <div className="p-3 border-t border-slate-800">
                        <button onClick={addModule} className="w-full py-2.5 border border-dashed border-slate-700 rounded-lg text-slate-400 text-xs font-medium hover:border-amber-500 hover:text-amber-400 hover:bg-amber-500/5 transition-all flex items-center justify-center gap-2">
                    ...
                    <div className="max-w-4xl mx-auto space-y-8 pb-24">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="group relative">
                                ...
                                <input
                                    className="w-full bg-transparent border-0 border-b-2 border-slate-700 text-3xl font-bold text-white focus:border-indigo-500 focus:ring-0 px-0 py-2 placeholder-slate-700 transition-colors"
                            ...
                            <nav className="-mb-px flex gap-6">
                                {([['content', 'Nội dung', FileText], ['settings', 'Cài đặt', Save], ['preview', 'Xem trước', Globe]] as const).map(([key, label, Icon]) => (
                                    <button key={key} onClick={() => setActiveTab(key)}
                                        className={`flex items-center gap-2 pb-3 px-1 border-b-2 text-sm font-medium transition-colors ${activeTab === key ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200 hover:border-slate-600'}`}>
                            ...
                            <div className="space-y-6">
                                ...
                                <section className="bg-[#0B1120] rounded-xl border border-slate-800/60 overflow-hidden shadow-xl">
                                    <div className="p-4 border-b border-slate-800 bg-[#0F1629] flex justify-between items-center">
                                        <h3 className="font-bold text-white flex items-center gap-2 text-sm">
                                            <div className="p-1 bg-amber-500/20 rounded text-amber-400"><PlayCircle className="w-4 h-4" /></div>
                                            Video bài giảng
                                        </h3>
                                        <span className="text-xs font-mono text-slate-500 bg-slate-900 px-2 py-1 rounded border border-slate-800">MP4, WebM (Max 2GB)</span>
                                    </div>
                                    <div className="p-8 bg-[#0B1120]">
                                        <div className="border-2 border-dashed border-slate-700 rounded-xl p-10 text-center hover:border-indigo-500 hover:bg-indigo-500/5 transition-all cursor-pointer group">
                                            <div className="w-14 h-14 bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-indigo-600 transition-all border border-slate-700 group-hover:border-indigo-500">
                                                <Plus className="w-6 h-6 text-slate-400 group-hover:text-white" />
                                            </div>
                                            <p className="text-slate-200 font-semibold mb-1">Kéo thả video vào đây</p>
                                            <p className="text-sm text-slate-500 mb-4">hoặc nhấn để chọn tệp từ máy tính</p>
                                            <button className="px-5 py-2 bg-slate-800 hover:bg-slate-700 text-white text-sm font-medium rounded-lg border border-slate-700">Chọn tệp tin</button>
                                        </div>
                                    </div>
                                </section>
                                ...
                                <section className="bg-[#0B1120] rounded-xl border border-slate-800/60 overflow-hidden shadow-xl">
                                    ...
                                    <div className="px-2 py-2 border-b border-slate-800 flex flex-wrap gap-1 bg-[#131b2e]">
                                        ...
                                        {['B', 'I', 'U'].map(f => (
                                            <button key={f} className="px-2 py-1 text-slate-400 hover:text-white hover:bg-white/10 rounded text-xs font-bold transition-colors">{f}</button>
                                        ))}
                                        <div className="w-px h-5 bg-slate-700 mx-1 my-auto" />
                                        {['≡', '1.', '{ }', '🔗', '🖼'].map((sym, i) => (
                                            <button key={i} className="px-2 py-1 text-slate-400 hover:text-white hover:bg-white/10 rounded text-xs transition-colors">{sym}</button>
                                        ))}
                                    </div>
                                ...
                                <section className="bg-[#0B1120] rounded-xl border border-slate-800/60 overflow-hidden shadow-xl">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 h-60 divide-y lg:divide-y-0 lg:divide-x divide-slate-800">
                                        ...
                                        <div className="bg-[#1e1e1e] p-4 font-mono text-xs overflow-auto">
                                            ...
                                            <div className="pl-4 text-slate-300">    print(<span className="text-green-400">"Test passed!"</span>)</div>
                                        </div>
                                    </div>
                                </section>
                                ...
                                <section className="bg-[#0B1120] rounded-xl border border-slate-800/60 overflow-hidden shadow-xl">
                                    <div className="p-4 border-b border-slate-800 bg-[#0F1629] flex justify-between items-center">
                                        ...
                                        <button className="text-xs bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 border border-indigo-500/20 px-3 py-1.5 rounded-full font-medium transition-colors flex items-center gap-1">
                                    ...
                                    <div className="p-5 bg-[#0B1120]">
                                        <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700/50 hover:border-slate-600 transition-colors group">
                                            <div className="flex justify-between mb-3">
                                                ...
                                                <button className="text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-4 h-4" /></button>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <button className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-1 mt-2 font-medium"><Plus className="w-3 h-3" /> Thêm đáp án</button>
                                            </div>
                                        </div>
                                    </div>
                                </section>
