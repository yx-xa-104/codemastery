...
const MOCK_THREADS = [
    ...
    { id: 4, name: 'Nguyễn Thị Lan', avatar: 'N', course: 'JS Nâng cao', preview: 'Thầy cho em hỏi về Promise.all vs Promise.race...', time: '5 giờ', unread: 0, bg: 'bg-purple-600' },
            ...
            <div className="-mx-6 -my-6 lg:-mx-8 h-[calc(100vh-8rem)] flex border border-indigo-900/30 rounded-xl overflow-hidden bg-[#0B1120]">
                    ...
                    <div className="flex-1 overflow-y-auto divide-y divide-indigo-900/20">
                        {MOCK_THREADS.map(thread => (
                            <button
                                ...
                                className={`w-full p-4 text-left hover:bg-white/5 transition-colors ${activeThread.id === thread.id ? 'bg-indigo-600/10 border-r-2 border-indigo-500' : ''}`}
                            >
                                <div className="flex items-center gap-3">
                                    <div className={`size-9 rounded-full ${thread.bg} flex items-center justify-center text-xs font-bold text-white shrink-0`}>
                                        {thread.avatar}
                                    </div>
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    ...
                    <div className="h-14 border-b border-indigo-900/30 flex items-center justify-between px-5">
                        <div className="flex items-center gap-3">
                            <div className={`size-8 rounded-full ${activeThread.bg} flex items-center justify-center text-xs font-bold text-white`}>
                                {activeThread.avatar}
                            </div>
                            <div>
                        ...
                        <button className="flex items-center gap-1.5 text-xs text-amber-400 hover:text-amber-300 bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-full transition-colors">
                    ...
                    <div className="p-4 border-t border-indigo-900/30">
                        <div className="flex items-end gap-3">
                            <textarea
                                ...
                                rows={1}
                                placeholder="Nhắn tin cho học viên..."
                            ...
                            <button
                                onClick={handleSend}
                                className="size-10 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl flex items-center justify-center transition-colors shrink-0"
                            >
