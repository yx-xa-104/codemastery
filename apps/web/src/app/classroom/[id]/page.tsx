        ...
        <div className="flex h-screen bg-[#010816] text-slate-100 overflow-hidden font-sans">
            ...
            <div className="flex-1 flex flex-col h-full">
                ...
                <header className="h-16 bg-[#0B1120]/80 backdrop-blur-md border-b border-indigo-900/30 flex justify-between items-center px-6 z-20">
                    ...
                    <div className="flex items-center gap-3">
                        <button className="relative size-9 bg-[#010816] rounded-full border border-slate-700 hover:border-amber-500/50 text-slate-400 hover:text-amber-400 flex items-center justify-center transition-colors">
                ...
                <main className="flex-1 flex overflow-hidden">
                            ...
                            <div className="bg-linear-to-r from-indigo-900/40 to-[#0B1120] border border-indigo-500/30 rounded-xl p-6 shadow-lg relative overflow-hidden">
                                ...
                                <div className="flex gap-3">
                                    {['De_cuong_on_tap.pdf (2.4MB)', 'Link đăng ký nhóm'].map(file => (
                                        <button key={file} className="flex items-center gap-2 px-3 py-2 bg-[#111827] hover:bg-slate-700 border border-slate-700 rounded-lg text-xs text-slate-300 transition-all">
                                    ...
                                    <div className="flex justify-between items-center mt-2">
                                        <div className="flex gap-3 text-slate-400">
                                            {[Image, Code, LinkIcon].map((Icon, i) => (
                                                <button key={i} className="hover:text-amber-400 transition-colors">
                                                    <Icon className="w-4 h-4" />
                                                </button>
                                            ))}
                                        </div>
                                        <button
                                            ...
                                            className="flex items-center gap-2 px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-medium rounded-lg transition-colors"
                                        >
                            ...
                            <div className="flex gap-1">
                                {[['all', 'Tất cả'], ['homework', 'Bài tập'], ['discussion', 'Thảo luận']].map(([key, label]) => (
                                    <button
                                        ...
                                        className={`text-xs font-medium px-3 py-1.5 rounded-full transition-colors ${activeTab === key ? 'text-white bg-indigo-600' : 'text-slate-400 hover:text-white'}`}
                                    >
                                        {label}
                                    </button>
                                ))}
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-5 mt-4 pt-3 border-t border-slate-800">
                                                <button className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-amber-400 transition-colors">
                    ...
                    <aside className="w-64 bg-[#0B1120] border-l border-indigo-900/30 flex-col hidden lg:flex">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            ...
                            <button className="w-full text-center text-xs text-slate-500 hover:text-indigo-400 py-2 border border-dashed border-slate-800 rounded hover:border-indigo-500/30 transition-colors">
