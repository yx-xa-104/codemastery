...
export function Sidebar({ courseTitle, modules, courseSlug, isOpen }: SidebarProps) {
    ...
    return (
        <aside
            className={`fixed top-14 left-0 z-40 h-[calc(100vh-3.5rem)] w-80 bg-[#0B1120] border-r border-slate-800 flex flex-col transition-transform duration-300 lg:translate-x-0 ${
                        ...
                        >
                            ...
                            <ChevronDown
                                className={`w-4 h-4 text-slate-500 transition-transform ${
                                    expandedModules[mod.id] ? "rotate-180" : ""
                                }`}
                            />
                        </button>
